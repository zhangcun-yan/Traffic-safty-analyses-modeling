...
def Ture_veh_pairs(combinations1,trajectory_data):
    ...
    for id in range(0,len(combinations1)-1,1):
        ...
        min_f_veh_frame_time = min(F_vehicle_frame_time)-20
        max_f_veh_frame_time = max(F_vehicle_frame_time)+20
        ...
        second_vehicle_inf = second_vehicle_inf[(second_vehicle_inf['frame_time']>=min_f_veh_frame_time)&(second_vehicle_inf['frame_time']<=max_f_veh_frame_time)]
        # if np.size(same_time_frame)<=0:
            # print(same_time_frame)
        if len(second_vehicle_inf)<=0:
    ...
    # combinations1.to_csv(veh_pair_save_path, index=False, header=True)
    return combinations1


def PET_function(combination,trajectory_data,Columns_name,Dis_threshold,df_all_veh_pairs):
    "the function main to calculate the PET value from trajectories"
    # df_all_veh_pairs = pd.DataFrame(columns=Columns_name)
    # for id in range(0,len(combinations1),1):
        # start_time = time.time()
    # print(str(id)+'/'+str(len(combination)))
    F_veh_id = combination[0]
    S_veh_id = combination[1]
    first_vehicle_inf = trajectory_data[trajectory_data['vehicle_id']==F_veh_id]
    second_vehicle_inf = trajectory_data[trajectory_data['vehicle_id']==S_veh_id]
    "Search the same frame time of the two vehicles"
    second_vehicle_inf = second_vehicle_inf.reset_index(drop=True)
    ...
    for frame_time_id in first_veh_frme_times:
        ...
        # here is a logstic erro need be selove
        second_vehicle_inf = second_vehicle_inf[(second_vehicle_inf['frame_time']>=Pre_frame_time)&(second_vehicle_inf['frame_time']<=Pro_frame_time)]
...
def Calculate_PET_value(trajectory_path,Columns_name,Dis_threshold):
    ...
    trajectory_data = trajectory_data.drop(trajectory_data[trajectory_data['vehicle_type'] == ' Pedestrian'].index)
    ...
    df_all_veh_pairs = pd.DataFrame(columns=Columns_name)
    # for combinations_row in combinations1:
    #     print(combinations_row)
    results_veh_pairs = [Pool.apply(PET_function,args =(combinations_row,trajectory_data,Columns_name,Dis_threshold,df_all_veh_pairs)) for combinations_row in combinations1]
...
output_path = r'E:/CodeResource/000_Traffic_conflict_risk_analysis/Data_clearning/Data_set/Denoising/WN/test_pet'
# veh_pair_save_path = r'E:/CodeResource/000_Traffic_conflict_risk_analysis/Data_clearning/Data_set/Conflict_index/PET/WN/veh_pair/veh_pair.csv'
# veh_pair_turth = pd.read_csv(veh_pair_save_path)
